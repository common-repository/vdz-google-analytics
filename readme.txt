=== VDZ Google Analytics or Google Tag Manager / GTM ===
Contributors: vadim8vz
Donate link:  http://online-services.org.ua#Donate-vdz-google-analytics
Tags: google analytics, google, analytics, marketing, wordpress plugin, tag, tag manager, google tag manager, gtm
Requires at least: 5.7.0
Requires PHP: 7.0
Tested up to: 5.9.1
Stable tag: 1.6.10
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

Simple add Google Analytics or Google Tag Manager (GTM) code on your site + Events for Contact Form 7 plugin

== Description ==

**БЕЛОРУСАМ - ЭТО НЕ ВАША ВОЙНА**

https://www.youtube.com/watch?v=u5XleFZkRng

**Вернись живым из Украины**

https://www.youtube.com/watch?v=A4Z-oX-7m_c

**УКРАЇНА - Є І БУДЕ ЗАВЖДИ**

https://www.youtube.com/watch?v=XJbVVMIZvVQ

Just install and use :) 
Simple Google Tag Manager (GTM) or Universal Google Analytics plugin.
After added Google Analytics ID - select where show your code in Head or Footer section.
Add Events for Contact Form 7 plugin
Add Events for [VDZ CallBack Plugin](https://wordpress.org/plugins/vdz-call-back/)

**Check out other plugins by VDZ:**

* **[VDZ CallBack Plugin](https://wordpress.org/plugins/vdz-call-back/)** 
* **[VDZ Yandex Metrika](https://wordpress.org/plugins/vdz-yandex-metrika/)** 
* **[VDZ Google Analytics](https://wordpress.org/plugins/vdz-google-analytics/)** 
* **[VDZ VERIFICATION](https://wordpress.org/plugins/vdz-verification/)**
* **[VDZ Scroll Up](https://wordpress.org/plugins/vdz-scroll-up/)**
* **[VDZ Support SVG](https://wordpress.org/plugins/vdz-support-svg/)**
* **[VDZ Contact Us](https://wordpress.org/plugins/vdz-contact-us/)** 
* **[VDZ Yandex Share](https://wordpress.org/plugins/vdz-yandex-share/)** 
* **[VDZ TRANSLIT](https://wordpress.org/plugins/vdz-translit/)** 
* **[VDZ Robots.txt](https://wordpress.org/plugins/vdz-robotstxt/)** 
* **[VDZ Monobank](https://wordpress.org/plugins/vdz-monobank/)** 
* **[VDZ Content Navigation](https://wordpress.org/plugins/vdz-content-navigation/)** 
* **[VDZ SHOW MORE](https://wordpress.org/plugins/vdz-show-more/)** 
* **[VDZ Ringostat Contacts](https://wordpress.org/plugins/vdz-ringostat-contacts/)** 
* **[VDZ Custom Simple CSS](https://wordpress.org/plugins/vdz-simple-css/)** 

== Screenshots ==
1. plugin settings page
2. Code on Front in HEAD (Default)
3. Code on Front in Footer

== Installation ==
<p>
Just upload the plugin from your Wordpress Plugins menu and activate it or download and upload to your WP Plugin directory (/wp-content/plugins/).
</p>


== Upgrade Notice ==
Add new version 1.6.10

== Changelog ==

= 1.6.10 =
Update ReadMe
Tested up to: 5.9.1

= 1.6.9 =
Tested up to: 5.9

= 1.6.8 =
Update ReadMe

= 1.6.7 =
Update ReadMe

= 1.6.6 =
Update API

= 1.6.5 =
Update notification

= 1.6.4 =
Update Readme

= 1.6.3 =
Tested up to: 5.8.1

= 1.6.2 =
Change ReadMe

= 1.6.1 =
Fix by phpcs

= 1.6.0 =
Add more Events for ContactForm 7
SEND_REQUEST
SEND_MAIL
SEND_ERROR

= 1.5.5 =
Remove option & fix scripts

= 1.5.4 =
Fix

= 1.5.3 =
Tested up to WP 5.8

= 1.5.2 =
Fix Settings all links

= 1.5.1 =
Fix Settings link

= 1.5.0 =
Update for new version Requires at least: 5.7.0 and Up

= 1.4.7 =
Tested up to WP 5.7.2

= 1.4.6 =
Tested up to WP 5.7.1

= 1.4.5 =
Tested up to WP 5.6.1

= 1.4.4 =
Tested up to WP 5.6

= 1.4.3 =
Add Events for Contact Form 7 plugin

= 1.4.2 =
Tested up to WP 5.5.1

= 1.4.1 =
Tested up to WP 5.5

= 1.4.0 =
Add Google Tag Manager code

= 1.3.5 =
Tested up to WP 5.4.2

= 1.3.4 =
Tested up to WP 5.4.1

= 1.3.3 =
Tested up to WP 5.4

= 1.3.2 =
Update PHP version

= 1.3.1 =
fix update api 

= 1.3.0 =
Add update api 

= 1.2.15 =
Tested up to WP 5.3.2

= 1.2.14 =
Tested up to WP 5.3

= 1.2.13 =
Tested up to WP 5.2.4

= 1.2.12 =
Check Working in WP 5.2.3

= 1.2.11 =
Check Working in WP 5.2

= 1.2.10 =
Check Working in WP 4.9.8

= 1.2.9 =
Check Working in WP 4.9.5

= 1.2.8 =
Check Working in WP 4.9

= 1.2.7 =
Check Working in WP 4.8.3

= 1.2.6 =
Check Working in WP 4.8.2

= 1.2.5 =
Check Working in WP 4.7.5

= 1.2.4 =
Check Working in WP 4.7.4

= 1.2.3 =
fix Contributors

= 1.2.2 =
Check Working in WP 4.7.3

= 1.2.1 =
Add links

= 1.2 =
Disable plugin if is not supported wordpress version

= 1.1 =
WordPress 4.7 compatibility
Add api

= 1.0 =
First release.